...
import pymongo
from bson.binary import Binary # NEW: For Cloud-Native asset storage
...
def init_db():
    ...
    db.assets.create_index("name", unique=True) # Index for Cloud-Native Assets
...
# --- CLOUD-NATIVE STORAGE LOGIC (REMOVED LOCAL STORAGE) ---
def save_card_to_db(emp_id, pdf_bytes, username, family_members, policy_no="UNKNOWN", card_type="BASE", company_name=None):
    ...
    if R2_ENABLED:
        ...
        # Fallback: Store directly in MongoDB if R2 isn't configured
            ...
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                if opt_merge:
                    ...
                    for eid, group in family_groups.items():
                        ...
                        save_name = f"{eid}_Family_ECard.pdf" if opt_rename else f"Processed_Family_{eid}.pdf"
...
with main_tab7:
    ...
    if master_file:
        ...
        except Exception as e: df_fam = pd.DataFrame()
